...
대사에 연기 지시를 붙이면, 같은 문장이 완전히 다른 사람이 된다.

```
(달래듯)              일어나. 벌써 일곱 시야.
(버럭)                일어나. 벌써 일곱 시야.
(밤새 울어서 목이 다 쉰)  일어나. 벌써 일곱 시야.
```

세 줄 다 같은 문장이고, 같은 보이스 모델이다. 바뀐 건 괄호 안뿐.

지시는 **한글로 고르면 된다.** `(버럭)`은 보낼 때 `[shouting][angry]`로 바뀐다.
사전에 없는 말은 그대로 전달되는데, Fish Audio가 자연어 연기 지시를 받아주기 때문이다.

**각자 자기 컴퓨터에서, 자기 API 키로 돌리는 도구다.** 공용 서버 같은 건 없다.
서버는 `127.0.0.1`에만 바인딩하고, 키는 브라우저로 나가지 않는다.

처음이면 **[docs/시작하기.md](docs/시작하기.md)** 부터. 키 발급까지 5분이면 된다.
...
- 웹 UI 오른쪽 팔레트에서 **한글로 고른다**. 검색도 한글·영어 둘 다 된다
- 지시는 **뒤따르는 대사 전체**에 걸린다. 여러 개 겹쳐도 된다
- 사전에 없는 한글은 그대로 전달된다 — `(새벽에 잠꼬대하듯 웅얼거리는)` 같은 것도 동작한다
- 83개 이상 언어

전체 목록과 매핑은 [`docs/tags.md`](docs/tags.md).
사전은 [`src/tags.mjs`](src/tags.mjs) 한 곳이라, 거기 한 줄 넣으면 UI·CLI에 같이 반영된다.
